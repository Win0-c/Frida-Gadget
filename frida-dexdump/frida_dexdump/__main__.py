...
class DexDumpApplication(ConsoleApplication):
    ...
    def _add_options(self, parser):
        ...
        parser.add_argument("--maps", help="Save runtime memory maps beside dumped DEX files.",
                            action='store_true', dest="dump_maps", default=False)
    ...
    def _initialize(self, parser, options, args):
        ...
        self.dump_maps_enabled = options.dump_maps
        # spawn mode
    ...
    def _start(self):
        ...
        if self.dump_maps_enabled:
            self.dump_maps()
        self.dump()
        self._exit(0)

    def dump_maps(self):
        logger.info("[+] Capturing runtime memory maps...")
        os.makedirs(self.output, exist_ok=True)
        maps = self.agent.enumerate_maps()
        json_path = os.path.join(self.output, "maps.json")
        text_path = os.path.join(self.output, "maps.txt")
        with open(json_path, "w", encoding="utf-8") as out:
            json.dump(maps, out, indent=2)
        ...
        logger.info("[+] Saved maps: {}, {}".format(json_path, text_path))
